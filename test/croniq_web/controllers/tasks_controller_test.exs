defmodule CroniqWeb.TasksControllerTest do
  ...
  describe "task pages" do
    ...
    test "Pagination: only page_size tasks are shown on a page", %{conn: conn, user_token: user_token, user: user} do
      ...
      assert Floki.text(Floki.find(parsed, "[data-test=id-cell]") |> Enum.at(0)) == Integer.to_string(Enum.at(tasks, 10).id)
    end

    test "Pagination: page_size selection works correctly", %{conn: conn, user_token: user_token, user: user} do
    ...
    test "Pagination: selected page_size is shown in select", %{conn: conn, user_token: user_token, user: user} do
      ...
      parsed = Floki.parse_document!(response)
      selected = Floki.find(parsed, "select[name=page_size] option[selected]") |> Floki.attribute("value")
      assert selected == ["20"]
    end
  end
